  ...
  return (
    <div className={cn({ 'opacity-30': disabled })}>
      <button
        ...
        onClick={isUtxo ? undefined : onToggle}
        disabled={disabled}
      >
        ...
        <div className="min-w-0 flex-1">
          ...
          <div className="text-thor-gray truncate text-xs font-medium">{truncate(account.address)}</div>
        </div>
        <div className="flex items-center gap-1">
          ...
          {isUtxo ? (
            <button
              onClick={e => {
                e.stopPropagation()
                openDialog(Send, { initialToken: nativeToken, account })
              }}
              className="text-thor-gray hover:text-leah cursor-pointer"
              aria-label={`Send ${chainName}`}
            >
              <Icon name="send" className="size-6" />
            </button>
          ) : isExpanded ? (
            <ChevronUp className="text-thor-gray size-4" />
          ) : (
            <ChevronDown className="text-thor-gray size-4" />
          )}
        </div>
      </button>
      ...
      {!isUtxo && isExpanded && (
        <div>
          {tokens.filter(t => t.amount > 0).length > 0 ? (
            ...
            <div className="text-thor-gray px-4 py-1 text-xs">No tokens found</div>
          )}
          <div className="border-t py-1">
            <button
              className="text-thor-gray hover:text-leah mx-4 flex w-[calc(100%-2rem)] cursor-pointer items-center justify-center rounded-xl border py-1.5 text-sm"
              onClick={() => {
