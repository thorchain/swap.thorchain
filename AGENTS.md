...
- `src/lib/agent/skills.ts` — published agent skills (one per capability area, YAML frontmatter); routes and the skills index are generated from `AGENT_SKILLS`.
- `src/lib/agent/markdown-pages.ts` — markdown twins: `.md` appended to any content page URL, plus `Accept: text/markdown` negotiation, resolved in `src/proxy.ts`.
