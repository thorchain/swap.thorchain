...
- Only edit `src/i18n/messages/en.json` by hand; every other locale is generated by `tools/i18n-translate.mjs`. Translations edited by native speakers are detected automatically and never overwritten — see `docs/localization.md` before touching the translator, the state files, or the workflow.
